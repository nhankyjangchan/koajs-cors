...
- **Type Definitions**: The `Plugin.Headers` type has been removed and replaced by a separate `Headers` interface with an index signature of type `string`. Type annotations have been updated throughout the package to reflect this change.
- **Moving `applyHeader` to Separate File**: The `applyHeader` function, previously declared directly in the middleware body, has been moved to a separate file, `src/lib/utils/applyHeader.ts`. The function now declares four parameters: the `ctx` context object, the headers CORS `header` object, and the previous header name and value parameters. This change will slightly improve performance, as it eliminates the previously required memory allocation for function creation on each request.
- **`mergeHeadersWithError` Function Refactoring**: The `mergeHeadersWithError` method has been refactored. The `corsHeaders` parameter has been renamed to `headers` and has received the updated `Headers` type instead of the old `Plugin.Headers`.
- **`cors` Middleware Factory Refactoring**: Updated `applyHeader` calls to reflect the change in its signature. Removed unnecessary `await` statements in places where waiting for the next middleware in the chain is not required. Rewritten the conditional block for skipping CORS header setting conditionally from `shouldSkip`; now the `await` statement only applies to `Promise-like` return values.
- **Documentation**:
    - **`docs/` Folder**: A `docs/` directory has been created, containing the full plugin type declaration and the full changelog file. The changelog file uploaded to npm is minified. This change is future-proofed and intended for viewing the types directly on GitHub.
    - **`README.md` File**: The `Options` section in `README.md` has been minified for easier comprehension. Links to header specifications have been updated. The `Usage` section has been updated, and a link to documentation has been added. These changes are cosmetic.
